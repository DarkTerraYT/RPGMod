    ...
    public class MasteryUI : MonoBehaviour
    {
        ...
        public ModHelperPanel OpenMasteryMenu(List<TowerXPData> data, string search = "")
        {
            ...
            if (!unitedXPMenu)
            {
                ...
                var text_ = universalXP.AddText(new("Text_Universal_XP", 0, -200, 600), $"{RpgUserData.UniversalXP} Universal XP", 70);
                    ...
                    if (AddedIds.Contains(data.BaseId))
                    {
                        ...
                        if (!unitedXPMenu)
                        {
                            if (RpgUserData.HasUniversalXP)
                            {
                                ...
                                uniXP.GetDescendent<ModHelperText>("Text").SetText(RpgUserData.UniversalXP.ToString());
                            }
                            ...
                            {
                                xpText.SetText($"{Math.Truncate(data.XP)} EXP");
                            }
                            else
                            {
                                xpText.SetText($"{Math.Truncate(data.XP)} / {data.XPToLevelUp} EXP");
                            }
                        ...
                        else
                        {
                            ...
                            var xpText = info.GetDescendent<ModHelperText>("Text_XP");
                            xpText.SetText(RpgUserData.UnitedXP[data.BaseId].ToString());
                        }
                    }
                }
            }
        ...
        private static ModHelperPanel CreateXPDataPanel(TowerXPData data)
        {
            ...
            var towerIcon = Game.instance.model.GetTowerFromId(data.BaseId).icon;
            ...
            if (!unitedXPMenu)
            {
                var xpText = panel.AddText(new("Text_XP", 0, panel.RectTransform.rect.top + 160, 800, 150), $"{Math.Truncate(data.XP)} / {data.XPToLevelUp} EXP", 50);
                xpText.Text.color = Color.green;
            ...
            {
                var xpText = panel.AddText(new("Text_XP", 0, panel.RectTransform.rect.top + 160, 800, 150), $"{RpgUserData.UnitedXP[data.BaseId]}", 50);
                xpText.Text.color = new(1, 0.55f, 0);

                var levelText = panel.AddText(new("Text_Level", 0, panel.RectTransform.rect.top + 80, 760, 150), $"Default Level: {data.Level}", 50);
                if (data.Level == 0)
                {
                    levelText.Text.color = Color.black;
                }
